...
void addNode::insertHead(NodeTreasure *&head, int data){
    NodeTreasure *n = new NodeTreasure(data);
    n -> next = head;
    head = n; 
}
void addNode::print(NodeTreasure *head) {
    while(head != NULL) {
        cout << head->data << " ";
        head = head ->next;
    }
...
bool checkBool::checkTreasure(NodeTreasure* head){
    bool checkx = false;
    bool checky = false;
    bool checkz = false;
    NodeTreasure* current = head;
    while (current != NULL) {
        if (current ->data == 95) {
            checkx = true;
        }
        else if(current ->data == 96) {
            checky =true;
        }
        else if(current ->data == 97) {
            checkz = true;
        }
        current = current->next;
    }
...
bool checkBool::checkPaladinShield(NodeTreasure* head) {
    NodeTreasure* current = head;
...
bool checkBool::checkLancelot(NodeTreasure* head) {
    NodeTreasure* current = head;
    while (current != nullptr) {
        if (current->data == 96) {
            return true;
...
bool checkBool::checkGuinevere(NodeTreasure* head) {
    NodeTreasure* current = head;
    while (current != nullptr) {
        if (current->data == 97) {
            return true;
...
bool ArmyKnights::adventure(Events * events){
   for(int i = 0; i < events->count(); i++){
        ...
        if (count_event == 11) {
            ...
            add -> insertHead(head, 96);
        }
        else if (events->get(i) == 97) {
            add -> insertHead(head, 97);
        }
            ...
            if(hasGuinevereHair() == false || hasLancelotSpear() == false || hasPaladinShield() == false){
...
bool ArmyKnights::hasPaladinShield () const {
    return check->checkPaladinShield(head);
}
bool ArmyKnights::hasLancelotSpear() const{
    return check->checkLancelot(head);
}
bool ArmyKnights::hasGuinevereHair() const{
    return check->checkGuinevere(head);
}
